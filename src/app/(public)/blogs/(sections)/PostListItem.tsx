    ...
    return (
        <article className="flex items-center gap-6 group">
            ...
            <div className="flex-grow">
                <Link href={`/blog?category=${post.category}`} className="text-sm font-bold text-nimo-yellow uppercase hover:underline">
                    {post.category}
                </Link>
                <h2 className="text-xl font-bold text-[var(--nimo-dark)] mt-1 leading-tight">
                    <Link href={`/blog/${post.slug}`} className="hover:text-nimo-yellow transition-colors duration-300">
                        {post.title}
                    </Link>
                </h2>
