...
export default function CheckoutPage() {
  ...
  const { isAuthenticated, isLoading: authLoading, authMethod, user } = useAuth(); // Ambil user dari useAuth
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!cartResponse.ok) {
        ...
        throw new Error((await cartResponse.json()).message || "Gagal memuat keranjang.");
      }
      ...
      toast.error("Gagal memuat data checkout.", { description: error instanceof Error ? error.message : "Tidak dapat terhubung ke server." });
      router.replace('/cart');
    } finally {
      setIsLoading(false);
    }
    ...
    return (
        <div className="container mx-auto py-8 px-4">
            <div className="text-center">
                <p className="text-muted-foreground">Silakan login untuk melanjutkan checkout.</p>
  ...
  if (!cart || cart.cartItems.length === 0) {
    ...
    return (
        <div className="container mx-auto text-center py-20">
            ...
            <p className="mt-2 text-muted-foreground">Anda akan diarahkan kembali ke halaman keranjang.</p>
