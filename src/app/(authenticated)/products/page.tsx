...
async function FilteredProductList({ category, search }: { category?: string, search?: string }) {
  // Bangun URL API secara dinamis dan lebih aman
  const params = new URLSearchParams();
  if (category) params.append('category', category);
  if (search) params.append('search', search);
  const queryString = params.toString();
  const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/products${queryString ? `?${queryString}` : ''}`;
    ...
    if (products.length === 0) {
      return (
        <div className="col-span-full text-center py-16">
          <p className="text-xl font-semibold text-[var(--nimo-dark)]">Oops! Tidak ada produk ditemukan.</p>
      ...
      <div className="container mx-auto py-8 px-4">
        <section>
          ...
          <div className="text-center bg-[var(--nimo-light)] dark:bg-card p-8 rounded-lg shadow-sm mb-8">
            <h1 className="text-4xl font-bold text-[var(--nimo-dark)]">{pageTitle}</h1>
