...
// Ikon baru untuk di dalam tombol toggle
...
const Navbar = () => {
  ...
  // --- Logic untuk Tema (Dark Mode) ---
  useEffect(() => {
    // Set tema dari localStorage atau preferensi sistem saat pertama kali load
  ...
  useEffect(() => {
    // Terapkan class 'dark' ke <html> dan simpan preferensi
    if (theme === "dark") {
  ...
  // --- Komponen Tombol Toggle (Desain Baru) ---
    ...
    <nav className="bg-[var(--nimo-light)] shadow-md sticky top-0 z-50 transition-colors duration-300">
        <div className="container mx-auto px-4 sm:px-6 py-3 flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center space-x-8">
                {menuItems.map((item) => (
                    <a key={item.href} href={item.href} onClick={(e) => { e.preventDefault(); handleMenuClick(item.href); }}
                        className={`text-[var(--nimo-dark)] font-medium transition-colors relative ${active===item.href ? "after:w-full text-nimo-yellow" : "" } hover:text-nimo-yellow after:content-[''] after:block after:h-0.5 after:bg-nimo-yellow after:transition-all after:duration-300 after:absolute after:left-0 after:-bottom-1 ${active===item.href ? "after:w-full" : "after:w-0 hover:after:w-full" } `}
                        >
                        {item.label}
                    </a>
                ))}
            ...
            <div className="flex items-center">
                <div className="md:hidden flex items-center">
                    <button className="text-[var(--nimo-dark)] focus:outline-none" onClick={()=> setMenuOpen(!menuOpen)} aria-label="Toggle menu">
        ...
        <div className={`md:hidden fixed inset-0 z-40 bg-black bg-opacity-40 transition-opacity duration-300 ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none" }`} onClick={()=> setMenuOpen(false)}
        />
        <div className={`md:hidden fixed top-0 right-0 w-4/5 max-w-xs h-full bg-[var(--nimo-light)] shadow-lg z-50 transform transition-transform duration-300 ${menuOpen ? "translate-x-0" : "translate-x-full" } `}>
